...
SystemMonitor::SystemMonitor() :
    ...
    lastProcessQueryTime(0), lastHistoryTime(0),
...
bool SystemMonitor::IsAutoStartEnabled() {
    HKEY hKey;
...
void SystemMonitor::EnableAutoStart(bool enable) {
    HKEY hKey;
...
void SystemMonitor::SaveSettings(int theme, float r, float g, float b) {
    HKEY hKey;
...
void SystemMonitor::LoadSettings(int& theme, float& r, float& g, float& b) {
    HKEY hKey;
...
std::vector<ProcessData> SystemMonitor::GetProcesses() {
    ULONGLONG currentTime = GetTickCount64();
...
bool SystemMonitor::KillProcess(DWORD processID) {
    HANDLE hProcess = OpenProcess(PROCESS_TERMINATE, FALSE, processID);
    if (hProcess == NULL) return false;
    bool result = TerminateProcess(hProcess, 1);
    CloseHandle(hProcess);
    if (result) Log("User terminated PID: " + std::to_string(processID));
    return result;
}
